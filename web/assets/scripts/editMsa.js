...
function loadData(){
    ...
    $.ajax({
        ...
        type: 'Get',
            ...
            //console.log(data);
            data.shift();
$("#selectClinic").val(data[0].branchId).trigger('change');;
...
//            console.log(iData);
      ...
      var batch,item;
       ...
       //console.log(batch);
            ...
            //alert(dat.brandName);
       addItem(item,batch,dat.quantity,dat.msu,dat.cp,dat.sp,dat.brandName,dat.brandId);         
...
function addItem(item,batch,qty,msu,cp,sp,brandName,brandId){
    //console.log(batch);
    
var rowCount = $('#tableItem tr').length;    
var newRow=$("<tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td hidden></td><td hidden></td><td width='25px'><button class='removeItem btn btn-block btn-danger remove btn-icon' title='Delete'><i class='fa fa-trash' aria-hidden='true'></i></button></td</tr>")
newRow.children().eq(0).text(rowCount);
newRow.children().eq(1).text(item.name);
newRow.children().eq(2).text(batch.name);
newRow.children().eq(3).text(msu);
newRow.children().eq(4).text(cp);
newRow.children().eq(5).text(sp);
newRow.children().eq(6).text(qty);
newRow.children().eq(7).text(brandName);
newRow.children().eq(8).text(brandId);
...
$("#addStock").on('click',function (){
    ...
    //console.log(item.itemName+" "+batch.batchName);
            //addItem(item,batch,dat.quantity,dat.msu,dat.cp,dat.sp);         

    addItem(item,batch,$("#qty").val(),batch.msu,batch.cp,batch.sp,$( "#selectBrand option:selected" ).text(),$( "#selectBrand").val());
});
...
var brandId=  $(this).find('td:nth-child(9)').text();   
  ...
  $.ajax({
            url: "stMSA",
            type: 'POST',
            data: {type:"editStockAllocation",purchaseId:pid,branchId:branchId,reference:reference,purchaseDate:purchaseDate,comment:comment,items:JSON.stringify(jsonItem)},
            success: function (data, textStatus, jqXHR) {
                if (data[0].status == "0") {
                    alert("Error");
                    console.log(data[0].errorMessage);
                ...
                alert("Updated");
                //console.log(data[0]);
                window.location="msaPending.jsp";
                $("#buttonSave").prop('disabled', false);
            },
          error: function (jqXHR, textStatus, errorThrown) {
                alert("Error");
