...
function loadItem(){
    $("#loading").show();
      $.ajax({
        ...
        type: 'GET',
        async: false,
        data: {type:"getTransferItemDetails"},
        success: function (data, textStatus, jqXHR) {
...
var newRow=$("<tr><td></td><td></td><td></td><td></td><td></td><td hidden></td><td width='25px'><button class='removeItem btn btn-block btn-danger delete btn-icon ' title='Delete'><i class='fa fa-trash' aria-hidden='true'></i></button></td</tr>")
...
$("#buttonSave").on('click',function(){
  ...
  $.ajax({
            url: "stBSA",
            type: 'POST',
            data: {type:"newStockTransfer",targetBranchId:branchId,reference:reference,purchaseDate:purchaseDate,comment:comment,items:JSON.stringify(jsonItem)},
            success: function (data, textStatus, jqXHR) {
                if (data[0].status == "0") {
                    alert("Error");
                    console.log(data[0].errorMessage);
