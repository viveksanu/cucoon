...
$(function(){
 $("#formFilter").unbind('submit').bind('submit', function () {
        ...
        url: 'stBSA',
        type: 'GET',
        data: $("#formFilter").serialize()+"&type=getStockTransferAll",
...
columnDefs: [{
        ...
        targets: [-1], render: function (a, b, data, d) {
            ...
            return "<button class='btn btn-block btn-danger disabled btn-icon' title='Delete'><i class='fa fa-trash' aria-hidden='true'></i></button>";
        }
    },{
        // puts a button in the last column
        targets: [-2], render: function (a, b, data, d) {
            ...
            return "<button class='btn btn-block btn-success disabled btn-icon' title='Edit'><i class='fa fa-pencil' aria-hidden='true'></i></button>";
        }
    }],
    ...
    $('#tableContent').on('click', '.vieww', function () {
     ...
     $.ajax({
        ...
        async: false,
        type: 'Get',
            data:{type:'viewStockTransfer',id:id,'ttype':-1},
            success: function (data, textStatus, jqXHR) {
                   $(".btn").prop('disabled', false);
...
//    '<"row"<"col-sm-5"i><"col-sm-7"p>>',      
  ...
  var id   = data["id"];
       var c = confirm("Confirm");
        ...
        type: 'GET',
            data:{id:id,type:"deleteStockTransfer"},
            success: function (data, textStatus, jqXHR) {
                   $(".btn").prop('disabled', false);
...
$('#tableContent').on( 'click', '.edit', function () {
           var data = table.row( $(this).parents('tr') ).data();
           window.open("editBsaStockTransfer.jsp?pid="+data["id"],'_self');//add value to session
    });
