...
//                       
        ...
        "columns": [
            ...
            { "data": null,
             "defaultContent": "<button class='btn btn-block btn-danger remove'>Delete Procedure</button>"
            }
...
$("#formNew").unbind('submit').bind('submit', function () {
   ...
   $.ajax({
        url: 'stLIH',
        type: 'POST',
            data:$("#formNew").serialize()+"&type=newLih",
            success: function (data, textStatus, jqXHR) {
                   $(".btn").prop('disabled', false);
            $("#loading").hide();
            if (data[0].status == "0") {
...
$("#formEdit").unbind('submit').bind('submit', function () {
   ...
   $.ajax({
        url: 'stLIH',
        type: 'POST',
            data:$("#formEdit").serialize()+"&type=editLih&id="+lihId,
            success: function (data, textStatus, jqXHR) {
                   $(".btn").prop('disabled', false);
            $("#loading").hide();
            if (data[0].status == "0") {
