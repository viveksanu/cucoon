...
function getInvoice(){
   ...
   $.ajax({
        ...
        type: 'POST',
        async: false,
            data:{tid:tid,type:"getInvoice",invoiceType:2},
            success: function (data, textStatus, jqXHR) {
                   $(".btn").prop('disabled', false);
...
$.each(invoiceMainParticularsJSON, function (i, dat) {
            ...
            var amount=0;
                $.each(procedureJSON, function (i, dat) {
                        ...
                        var newRow = $("<tr><td></td><td></td><td></td><td></td><td></td><td  style='width: 250px;' text-right></td></tr>");
                        newRow.children().eq(0).text(i+1);
                        newRow.children().eq(1).text(dat.date);
                        newRow.children().eq(2).text(dat.procedureName);
                        newRow.children().eq(3).text(Number(dat.amount).toFixed(2));
                        newRow.children().eq(4).text(Number(dat.discount).toFixed(2));
                        newRow.children().eq(5).text(Number(dat.netAmount).toFixed(2));
                        newRow.appendTo($("#tableProcedure"));
                        amount=amount+Number(dat.netAmount);
            });
            $("#prAmount").text(amount.toFixed(2));
            amount=0;
                   $.each(lihJSON, function (i, dat) {
                        ...
                        newRow.children().eq(0).text(i+1);
                        newRow.children().eq(1).text(dat.date);
                        newRow.children().eq(2).text(dat.lihName);
                        
                        newRow.children().eq(3).text(Number(dat.amount).toFixed(2));
                        newRow.children().eq(4).text(Number(dat.discount).toFixed(2));
                        newRow.children().eq(5).text(Number(dat.netAmount).toFixed(2));
                        newRow.appendTo($("#tableLih"));
                        amount=amount+Number(dat.netAmount);
            });
            $("#liAmount").text(amount.toFixed(2));            
            amount=0;
            
                               $.each(pharmacyJSON, function (i, dat) {
                
                        var newRow = $("<tr><td></td><td></td><td></td><td></td><td></td><td></td><td  style='width: 250px;' text-right></td></tr>");
                        newRow.children().eq(0).text(i+1);
                        newRow.children().eq(1).text(dat.date);
                        newRow.children().eq(2).text(dat.itemName);
                        newRow.children().eq(3).text(dat.quantity);
                        newRow.children().eq(4).text(Number(dat.amount).toFixed(2));
                        newRow.children().eq(5).text(Number(dat.discount).toFixed(2));
                        newRow.children().eq(6).text(Number(dat.netAmount).toFixed(2));
                        newRow.appendTo($("#tablePharmacy"));
                        amount=amount+Number(dat.netAmount);
            });
            $("#phTotalAmount").text(amount.toFixed(2));            
...
//       console.log(dat);
           ...
           $("#rbillAmount").text(amount.toFixed(2));
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert("Error");
            $("#loading").hide();
